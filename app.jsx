/**
 * SISTEMA: Control Meta Pro v5.2 (SQLite Only)
 * CARACTERÍSTICAS: Polling de alta frecuencia para sincronización entre auditores, selección masiva, sin truncado.
 */
...
const Dashboard = ({ userEmail, onLogout }) => {
    ...
    useEffect(() => {
        sync();
        const interval = setInterval(() => sync(true), 30000); // Polling cada 30 seg
    ...
    if (loading && !data.meta.length) return <div className="min-h-screen bg-black flex items-center justify-center font-black text-blue-500 uppercase animate-pulse">Iniciando SQLite Engine...</div>;
        ...
        <div className="min-h-screen bg-black text-white p-6 lg:p-12 font-sans animate-fade-in">
            <header className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
                <div className="bg-zinc-900 p-8 rounded-[2.5rem] border border-white/5 flex items-center justify-between shadow-2xl">
                    <div><p className="text-[10px] font-black text-blue-500 uppercase tracking-widest">Automatización</p><p className="text-xl font-black italic">{data.automation_active ? 'ACTIVA' : 'APAGADA'}</p></div>
            ...
            <div className="bg-zinc-900/50 p-6 rounded-[2.5rem] border border-white/5 mb-8 flex flex-wrap items-center gap-6 shadow-xl">
                ...
                <button onClick={async () => {
                    await fetch(`${API_URL}/ads/update`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: 'all', is_frozen: false, user: userEmail, log: "Realizó descongelado manual masivo" }) });
                    sync(true);
                }} className="text-[10px] font-black uppercase bg-zinc-800 px-6 py-4 rounded-2xl border border-white/5 hover:bg-zinc-700 transition-all tracking-widest">Descongelar Todos (Reset)</button>
            </div>
                            ...
                            {sortedData.map(ad => {
                                ...
                                return (
                                    <tr key={ad.id} className={`border-b border-white/5 hover:bg-white/[0.02] transition-colors ${s.is_frozen ? 'opacity-40' : ''}`}>
                                        ...
                                        <td className="p-6 text-center"><input type="number" className="bg-black border border-white/10 w-16 p-2 rounded text-center text-blue-500 font-black outline-none" value={s.limit_perc} onBlur={(e) => updateSetting(ad.id, 'limit_perc', parseFloat(e.target.value), `Cambió límite a ${e.target.value}% en AdSet ${ad.id}`)} /></td>
                                        ...
                                        <td className="p-6 text-center"><button onClick={() => updateSetting(ad.id, 'is_frozen', !s.is_frozen, `${!s.is_frozen ? 'Congeló' : 'Descongeló'} AdSet ${ad.id}`)} className={`p-3 rounded-xl transition-all ${s.is_frozen ? 'bg-blue-600 text-white shadow-lg' : 'bg-white/5 text-zinc-700'}`}>{s.is_frozen ? <Icon name="lock" size={16} /> : <Icon name="unlock" size={16} />}</button></td>
                                    </tr>
                                );
                            })}
                        </tbody>
