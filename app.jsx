...
const TurnSelector = ({ currentTurnos, availableTurns, onUpdate }) => {
    ...
    const activeList = currentTurnos ? currentTurnos.split(',').map(t => t.trim().toLowerCase()).filter(t => t) : [];
    ...
    const toggleTurno = (turnoName) => {
        const name = turnoName.toLowerCase();
        let newList;
        if (activeList.includes(name)) {
            newList = activeList.filter(t => t !== name);
        } else {
            newList = [...activeList, name];
        }
    ...
    return (
        <div className="relative" ref={containerRef}>
            <button
                ...
                className="bg-black/40 border border-white/10 p-2.5 rounded-xl text-[10px] font-black uppercase text-zinc-400 w-36 flex justify-between items-center hover:bg-black/60 transition-all"
            >
            ...
            <header className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8 text-left">
                ...
                <div className="bg-zinc-900/50 p-6 rounded-[2rem] border border-white/5 flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                        <button onClick={() => setShowLogs(!showLogs)} className="relative p-2 bg-white/5 rounded-xl transition-all">
                                    ...
                                    {sortedData.map(ad => {
                                        ...
                                        return (
                                            <tr key={ad.id} className={`border-b border-white/5 hover:bg-white/[0.01] transition-colors ${s.is_frozen ? 'opacity-30' : ''}`}>
                                                ...
                                                <td className="p-6">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <button
                                                            ...
                                                            className={`px-2 py-1.5 rounded-lg text-[8px] font-black uppercase transition-all ${active ? 'bg-zinc-800 text-zinc-500 hover:text-rose-500' : 'bg-emerald-600 text-white shadow-lg shadow-emerald-500/10'}`}
                                                        >
                                                            {active ? 'Apagar' : 'Prender'}
                                                        </button>
                                                    </div>
                                                </td>
                                                ...
                                                <td className="p-6 text-center">
                                                    <input type="number" className="bg-black border border-white/10 w-16 p-2 rounded-xl text-center text-blue-500 font-black outline-none focus:border-blue-500" value={s.limit_perc} onBlur={(e) => updateSetting(ad.id, 'limit_perc', parseFloat(e.target.value), `Ajustó límite a ${e.target.value}%`)} />
                                                </td>
                                                ...
                                                <td className="p-6 text-center">
                                                    <button onClick={() => updateSetting(ad.id, 'is_frozen', !s.is_frozen, `${!s.is_frozen ? 'Congeló' : 'Descongeló'}`)} className={`p-3 rounded-xl transition-all ${s.is_frozen ? 'bg-blue-600 shadow-lg shadow-blue-500/20' : 'bg-zinc-800 text-zinc-600'}`}>
                    ...
                    {Object.keys(data.turns).length > 0 ? Object.entries(data.turns).map(([name, config]) => (
                        <div key={name} className="bg-zinc-900/50 p-10 rounded-[2.5rem] border border-white/5 shadow-2xl">
                            <div className="flex items-center gap-3 mb-8">
                                <div className="bg-blue-600/10 p-3 rounded-[1.5rem]"><Icon name="Clock" className="text-blue-500" size={20} /></div>
                                <h2 className="text-lg font-black uppercase tracking-widest text-zinc-100">{name}</h2>
                            </div>
